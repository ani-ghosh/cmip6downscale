...
while(nrow(sidx) > 0){
  # lapply(1:nrow(sidx), getDataCMIP6, idx, downdir, silent=FALSE, overwrite=FALSE)
  library(future.apply)
  plan(multiprocess, workers = 12)
  future_lapply(1:nrow(idx), getDataCMIP6, idx, downdir, silent=FALSE, overwrite=FALSE, future.seed = TRUE)
...
# ff <- list.files(file.path(downdir, "CMIP6"), pattern = ".nc$", full.names = TRUE, recursive = TRUE)
